...
public class Demography extends Base {
...
public static void baseLineDemography() throws Exception {
	...
	findElements.get(patientSize - 1).click();
			...
			int randomIntValue = (int)( Math.floor( Math.random() + 1.5 ) );
			//int randomIntValue=((int) (Math.random() * (4 - 1))) + 1;
			...
			for(WebElement copyElement:list2) {
			 ...
			 String textIndicatio = list2.get(randomIntValue).getText();
		...
		//	List<String> stringList=elementManager.getElementByXpath(driver, "//div[4]/ul/li/span");
			...
			addAl.removeAll(Collections.singleton(""));
	         ...
	         String[] celldata2=cellData.split("/");
				
				for(String a:celldata2)
				{
					listIndi.add(a.trim());
				}
				...
				{
					 for(int k=7;k<11;k++) {
					 obj.setCellData("DEMOGRAPHY", 6, "Result", "ListItems Verified",k); 
							  }
				}
				...
				// cssSelector("[test-id='test_dd_input_Indication1_options']
				// ul.multiselect__content>li:nth-of-type(3)"))).click();
...
//			elementManager.waitWebDriver(driver, "button.button-primary[test-id='test_ms_btn_submit']");
			...
			if (driver.getTitle().equals("Baseline Plus - Maralixibat")) {
			 ...
			 // String[] arr= {tx,tx1,tx2,tx3};
			  ...
			  for(int j=7;j<9;j++) {
			  obj.setCellData("DEMOGRAPHY", 6, "Results", "pass",j); 
			  }
			  }
			  ...
			  obj.setTitleData("DEMOGRAPHY", 0, 6, "pass"); obj.setTitleData("DEMOGRAPHY",3, 6, "pass");
			  ...
			  driver.findElement(By.id("btnnext")).click();
				...
				if(textIndicatio.equals("ALGS")) {
					AlgsHistory algsPage=new AlgsHistory();
					
					algsPage.baselineALGShistory();
					...
					if(textIndicatio.equals("PFIC")) {
						PficHistory pficPage=new PficHistory();
						pficPage.baselinePFIChistory();
				}
